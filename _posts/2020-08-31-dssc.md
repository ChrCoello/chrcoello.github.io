...
In fine, the clustering process is useful to reduce the amount of model we need to train from `N` to `K`. We saw very early the obvious tradeoff that we will be confronted with :
 - we (hoped to) increase the accuracy of our forecast when having more and more clusters (with the extreme of having one model per NS)
 - we wanted to decrease the number of model by training one model 
