...
<figcaption>Schematic of the structure of the low voltage grid in Elvia. The timeseries represent the aggregated load over all the end users in one TF over a two-year period.An elephant at sunset</figcaption>
...
*Table 1. Example of end-user type in one substation. Three types of end-users are connected to this transformer. Based on the annual consumption, we assign the sector-label Industry to this substation.*

In the example of *Table 1*, we that the substation has 9 end-users connected to it with *Household* being the type with most end-users. But we also see that the annual consumption for the label *Industry* is higher than for *Household*, and there fore this substation will get the sector-label *Industry* attached to it.  
...
The advantage of these data-driven methods are that the number of cluster is a hyper-parameter that the user can set prior to running the algorithm. Interesting to see that choosing 3 clusters is the optimal number of cluster based on the elbow method (*Figure 3, left*), but we see that the "elbow" is very smooth and that changing metric (*Figure 3, right*) makes the elbow not detected. When clustering the data using K=3 and the K-means algorithm, the cluster generated are very similar to the clusters to the one obtained in the label-driven clusters (*Figure 2*).  

<img src="/images/2020-08-31-dssc/data_driven_cluster_elbow.png" width="600" class="center" alt="Label based clusters">  

*Figure 3. Data-based clustering and distribution of the labels in each data-driven clusters.* 

The data-driven clustering allows to have an additional degree of freedom, the number of clusters *K*.  This is essential to study more in detail how reducing the number of forecasting models influence the forecasting. Very early, the tradeoff (*Figure 4*) between number of models and accuracy was identified by thinking about the extreme cases:  
...
*Figure 4. Tradeoff between the number of forecasting models trained and the accuracy of the forecast*
...
In this case, we have optimized each model's hyperparamters by using cross-validation method. It is observed that default hyperparameters for CatBoost were already very close to the optimal set of hyperparameters obtained at the end of the lengthy CV run.
...
*Figure 5. Comparing models when predicting six different substations. Each subplot shows the predicted load of each model and the true load (black) for one forecast horizon (67 hours).*

**TODO COMMENT FIGURE 5**
...
*Table 2. Forecasting results obtained with four different models. The mean (and std) reported in the table are the mean (and std) over the 15 forecasts horizon. Each forecast horizon predicts 67 hours in the future.*

There are several interesting results within Table 2:  
 1. the metrics obtained for all three tree-based models are roughly similar, whereas Prophet is worse in 5 out of 6 cases. Not to enter in the details of Prophet, we generally didn't see the potential of using this model for short-term load forecasting. 
 2. the level of accuracy obtained for *house* and *industry* (~5%) is very promising, with feature load-7 being the feature with most importance with random forest. Would be interesting to look deeper at features, for example looking at [SHAP](https://github.com/slundberg/shap) values. 
...
Last but not least, I would like to thank the four students for their amazing contribution during these two months, where team work was reduced to discussion over the Internet and some very few (but very intense and constructive) physical meetings over the whiteboard.
