...
The advantage of these data-driven methods are that the number of cluster is a hyper-parameter that the user can set prior to running the algorithm. Interesting to see that choosing 3 clusters is the optimal number of cluster based on the elbow method ([REF]). When clustering the data using K=3 and the K-means algorithm, the cluster generated are very similar to the clusters to the one obtained in the label-driven clusters (*Figure 2*).  
